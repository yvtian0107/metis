...
var statusTransitions = map[string][]string{
	StatusUnpublished: {StatusPublished, StatusArchived},
	StatusPublished:   {StatusUnpublished, StatusArchived},
	StatusArchived:    {StatusUnpublished},
}
...
func (s *ProductService) CreateProduct(name, code, description string) (*Product, error) {
	...
	pubKey, encPrivKey, err := GenerateKeyPair(encKey)
	if err != nil {
...
func (s *ProductService) UpdateProduct(id uint, params UpdateProductParams) (*Product, error) {
...
func (s *ProductService) GetPublicKey(productID uint) (*ProductKey, error) {
...
func (s *PlanService) CreatePlan(productID uint, name string, constraintValues json.RawMessage, sortOrder int) (*Plan, error) {
	...
	plan := &Plan{
		ProductID:        productID,
		Name:             name,
		ConstraintValues: JSONText(constraintValues),
		SortOrder:        sortOrder,
	}
...
func (s *PlanService) UpdatePlan(id uint, name *string, constraintValues json.RawMessage, sortOrder *int) (*Plan, error) {
	...
	if constraintValues != nil {
		...
		plan.ConstraintValues = JSONText(constraintValues)
	}
...
func validateConstraintSchema(schema ConstraintSchema) error {
	...
	for _, m := range schema {
		...
		for _, f := range m.Features {
			...
			switch f.Type {
			case FeatureTypeNumber, FeatureTypeEnum, FeatureTypeMultiSelect:
				// valid
			default:
...
func validateConstraintValues(schemaJSON json.RawMessage, valuesJSON json.RawMessage) error {
	...
	// Build schema lookup
	schemaLookup := make(map[string]map[string]ConstraintFeature)
	for _, m := range schema {
		features := make(map[string]ConstraintFeature)
		for _, f := range m.Features {
			features[f.Key] = f
		}
...
func validateFeatureValue(feature ConstraintFeature, value any, moduleKey string) error {
	switch feature.Type {
