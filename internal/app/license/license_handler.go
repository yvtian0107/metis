...
func (h *LicenseHandler) Issue(c *gin.Context) {
	...
	if err != nil {
		if errors.Is(err, ErrProductNotFound) || errors.Is(err, ErrProductNotPublished) ||
			errors.Is(err, ErrLicenseeNotFound) || errors.Is(err, ErrLicenseeNotActive) ||
...
func (h *LicenseHandler) List(c *gin.Context) {
	...
	result := make([]LicenseResponse, len(items))
...
func (h *LicenseHandler) ListRegistrations(c *gin.Context) {
	...
	items, total, err := h.licenseSvc.ListLicenseRegistrations(LicenseRegistrationListParams{
		ProductID:  productID,
		LicenseeID: licenseeID,
		Available:  available,
		Page:       page,
