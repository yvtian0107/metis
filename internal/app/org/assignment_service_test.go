...
func newAssignmentService(db *database.DB) *AssignmentService {
	return &AssignmentService{
		...
		deptRepo: &DepartmentRepo{db: db},
		posRepo:  &PositionRepo{db: db},
	}
}
...
func TestAssignmentService_AddUserPosition(t *testing.T) {
	...
	// explicit primary
	dept3 := seedDepartment(t, db, "Sales", "sales", nil, nil, true)
...
func TestAssignmentService_AddUserPosition_ValidationErrors(t *testing.T) {
	...
	role := seedRole(t, db, "user")
	activeDept := seedDepartment(t, db, "Engineering", "eng", nil, nil, true)
	inactiveDept := seedDepartment(t, db, "Inactive", "inactive", nil, nil, false)
	activePos := seedPosition(t, db, "SE", "se", true)
	inactivePos := seedPosition(t, db, "InactivePos", "inactive_pos", false)
	user := seedUser(t, db, "u1", role.ID)
...
func TestAssignmentService_GetUserDepartmentScope(t *testing.T) {
	...
	activeChild := seedDepartment(t, db, "ActiveChild", "active_child", &parent.ID, nil, true)
	inactiveChild := seedDepartment(t, db, "InactiveChild", "inactive_child", &parent.ID, nil, false)
	_ = seedDepartment(t, db, "GrandChild", "grand_child", &activeChild.ID, nil, true)
	pos := seedPosition(t, db, "SE", "se", true)
	user := seedUser(t, db, "u1", role.ID)
