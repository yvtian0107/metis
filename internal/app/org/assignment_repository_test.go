...
func TestAssignmentRepo_AddPosition(t *testing.T) {
	...
	up := &UserPosition{UserID: user.ID, DepartmentID: dept.ID, PositionID: pos.ID}
...
func TestAssignmentRepo_AddPositionWithPrimary_DemoteExisting(t *testing.T) {
	...
	up := &UserPosition{UserID: user.ID, DepartmentID: dept2.ID, PositionID: pos.ID}
...
func TestAssignmentRepo_RemovePosition_AutoPromote(t *testing.T) {
	...
	pos := seedPosition(t, db, "SE", "se", true)
	user := seedUser(t, db, "u1", role.ID)
	primary := seedAssignment(t, db, user.ID, dept1.ID, pos.ID, true)
	secondary := seedAssignment(t, db, user.ID, dept2.ID, pos.ID, false)
...
func TestAssignmentRepo_RemovePosition_LastAssignment(t *testing.T) {
	...
	pos := seedPosition(t, db, "SE", "se", true)
	user := seedUser(t, db, "u1", role.ID)
	primary := seedAssignment(t, db, user.ID, dept.ID, pos.ID, true)
...
func TestAssignmentRepo_UpdatePositionWithPrimary(t *testing.T) {
	...
	user := seedUser(t, db, "u1", role.ID)
	up := seedAssignment(t, db, user.ID, dept.ID, pos1.ID, false)
...
func TestAssignmentRepo_UpdatePositionWithPrimary_SetPrimary(t *testing.T) {
	...
	user := seedUser(t, db, "u1", role.ID)
	seedAssignment(t, db, user.ID, dept1.ID, pos.ID, true)
	up2 := seedAssignment(t, db, user.ID, dept2.ID, pos.ID, false)
...
func TestAssignmentRepo_ListUsersByDepartment(t *testing.T) {
	...
	u2 := seedUser(t, db, "bob", role.ID)
	seedAssignment(t, db, u1.ID, dept.ID, pos.ID, true)
	seedAssignment(t, db, u2.ID, dept.ID, pos.ID, false)
...
func TestAssignmentRepo_CountByDepartments(t *testing.T) {
	...
	dept2 := seedDepartment(t, db, "Product", "prod", nil, nil, true)
	pos := seedPosition(t, db, "SE", "se", true)
	u1 := seedUser(t, db, "u1", role.ID)
	u2 := seedUser(t, db, "u2", role.ID)
	seedAssignment(t, db, u1.ID, dept1.ID, pos.ID, true)
	seedAssignment(t, db, u2.ID, dept1.ID, pos.ID, false)
	seedAssignment(t, db, u1.ID, dept2.ID, pos.ID, true)
