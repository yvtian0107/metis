...
func TestDepartmentRepo_CreateFindByIDFindByCode(t *testing.T) {
	...
	dept := &domain.Department{Name: "Engineering", Code: "eng", IsActive: true}
...
func TestDepartmentRepo_ListActive(t *testing.T) {
	...
	testutil.SeedDepartment(t, db, "Active", "active", nil, nil, true)
	testutil.SeedDepartment(t, db, "Inactive", "inactive", nil, nil, false)
...
func TestDepartmentRepo_HasMembers(t *testing.T) {
	...
	dept := testutil.SeedDepartment(t, db, "Dept", "dept", nil, nil, true)
	user := testutil.SeedUser(t, db, "u1", role.ID)
	testutil.SeedAssignment(t, db, user.ID, dept.ID, 1, false)
...
func TestDepartmentRepo_ListAllIDsWithParent(t *testing.T) {
	...
	parent := testutil.SeedDepartment(t, db, "Parent", "parent", nil, nil, true)
	testutil.SeedDepartment(t, db, "ActiveChild", "active_child", &parent.ID, nil, true)
	testutil.SeedDepartment(t, db, "InactiveChild", "inactive_child", &parent.ID, nil, false)
