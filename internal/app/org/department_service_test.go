...
func newDepartmentService(db *database.DB) *DepartmentService {
	return &DepartmentService{
		...
		assignRepo: &AssignmentRepo{db: db},
	}
}
...
func TestDepartmentService_Tree(t *testing.T) {
	...
	child := seedDepartment(t, db, "Child", "child", &parent.ID, nil, true)

	role := seedRole(t, db, "user")
	pos := seedPosition(t, db, "SE", "se", true)
	u1 := seedUser(t, db, "u1", role.ID)
	seedAssignment(t, db, u1.ID, parent.ID, pos.ID, true)
	seedAssignment(t, db, u1.ID, child.ID, pos.ID, false)
