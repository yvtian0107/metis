...
type AssignmentService struct {
	repo     *AssignmentRepo
	deptRepo *department.DepartmentRepo
	posRepo  *position.PositionRepo
}

func NewAssignmentService(i do.Injector) (*AssignmentService, error) {
	repo := do.MustInvoke[*AssignmentRepo](i)
	deptRepo := do.MustInvoke[*department.DepartmentRepo](i)
	posRepo := do.MustInvoke[*position.PositionRepo](i)
	return &AssignmentService{repo: repo, deptRepo: deptRepo, posRepo: posRepo}, nil
}

func (s *AssignmentService) GetUserPositions(userID uint) ([]domain.UserPositionResponse, error) {
	items, err := s.repo.FindByUserID(userID)
	if err != nil {
		return nil, err
	}
	result := make([]domain.UserPositionResponse, 0, len(items))
	for _, item := range items {
		resp := domain.UserPositionResponse{
			ID:           item.ID,
			UserID:       item.UserID,
...
func (s *AssignmentService) AddUserPosition(userID, deptID, posID uint, isPrimary bool) (*domain.UserPosition, error) {
	...
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, position.ErrPositionNotFound
		}
		return nil, err
	}
	if !pos.IsActive {
...
func (s *AssignmentService) ListDepartmentMembers(deptID uint, keyword string, page, pageSize int) ([]domain.MemberWithPositions, int64, error) {
