...
func seedServiceDefinitions(db *gorm.DB) error {
	...
	multiRoleParallelApprovalFormSchema := `{"version":1,"fields":[{"key":"title","type":"text","label":"申请标题","required":true,"validation":[{"rule":"required","message":"请输入申请标题"}],"width":"full"},{"key":"target_system","type":"text","label":"目标系统","description":"填写本次申请涉及的目标系统或资源名称。","required":true,"validation":[{"rule":"required","message":"请输入目标系统"}],"width":"half"},{"key":"time_window","type":"date_range","label":"时间窗口","description":"申请操作的起止时间。","required":true,"validation":[{"rule":"required","message":"请选择时间窗口"}],"width":"half","props":{"withTime":true,"mode":"datetime"}},{"key":"reason","type":"textarea","label":"申请原因","description":"说明发起本次申请的原因和背景。","required":true,"validation":[{"rule":"required","message":"请填写申请原因"}],"width":"full","props":{"rows":3}},{"key":"expected_result","type":"textarea","label":"期望结果","description":"填写通过审批后预期达到的目标状态。","required":true,"validation":[{"rule":"required","message":"请填写期望结果"}],"width":"full","props":{"rows":3}}],"layout":{"columns":2,"sections":[{"title":"申请基本信息","fields":["title","target_system","time_window"]},{"title":"申请详情","fields":["reason","expected_result"]}]}}`
	multiRoleParallelApprovalWorkflowJSON := `{"nodes":[{"id":"start","type":"start","position":{"x":400,"y":50},"data":{"label":"开始","nodeType":"start"}},{"id":"parallel_fork","type":"parallel","position":{"x":400,"y":200},"data":{"label":"并签拆分","nodeType":"parallel","gateway_direction":"fork"}},{"id":"approve_network","type":"approve","position":{"x":180,"y":400},"data":{"label":"网络管理员审批","nodeType":"approve","participants":[{"type":"position_department","department_code":"it","position_code":"network_admin"}]}},{"id":"approve_security","type":"approve","position":{"x":620,"y":400},"data":{"label":"安全管理员审批","nodeType":"approve","participants":[{"type":"position_department","department_code":"it","position_code":"security_admin"}]}},{"id":"parallel_join","type":"parallel","position":{"x":400,"y":600},"data":{"label":"并签汇聚","nodeType":"parallel","gateway_direction":"join"}},{"id":"approve_ops","type":"approve","position":{"x":400,"y":800},"data":{"label":"运维管理员最终审批","nodeType":"approve","participants":[{"type":"position_department","department_code":"it","position_code":"ops_admin"}]}},{"id":"end_completed","type":"end","position":{"x":400,"y":1000},"data":{"label":"审批完成","nodeType":"end"}},{"id":"end_rejected","type":"end","position":{"x":700,"y":700},"data":{"label":"审批驳回","nodeType":"end"}}],"edges":[{"id":"e1","source":"start","target":"parallel_fork"},{"id":"e3","source":"parallel_fork","target":"approve_network"},{"id":"e4","source":"parallel_fork","target":"approve_security"},{"id":"e5","source":"approve_network","target":"parallel_join","data":{"outcome":"approved"}},{"id":"e6","source":"approve_network","target":"end_rejected","data":{"outcome":"rejected"}},{"id":"e7","source":"approve_security","target":"parallel_join","data":{"outcome":"approved"}},{"id":"e8","source":"approve_security","target":"end_rejected","data":{"outcome":"rejected"}},{"id":"e9","source":"parallel_join","target":"approve_ops"},{"id":"e10","source":"approve_ops","target":"end_completed","data":{"outcome":"approved"}},{"id":"e11","source":"approve_ops","target":"end_rejected","data":{"outcome":"rejected"}}]}`
	seeds := []serviceSeed{
		{
			Name:              "Copilot 账号申请",
			...
			Actions: []ServiceAction{
				{
					...
					ConfigJSON: JSONField(`{"url":"https://httpbin.org/anything/db-backup-precheck","method":"POST","headers":{"Content-Type":"application/json","X-ITSM-Action":"precheck","X-ITSM-Service":"db-backup-whitelist"},"body":"{\"ticket_code\":\"{{ticket.code}}\",\"database_name\":\"{{ticket.form_data.database_name}}\",\"source_ip\":\"{{ticket.form_data.source_ip}}\",\"whitelist_window\":\"{{ticket.form_data.whitelist_window}}\",\"access_reason\":\"{{ticket.form_data.access_reason}}\"}","timeout":10,"retries":0}`),
				},
				{
					Name: "执行备份白名单放行", Code: "db_backup_whitelist_apply",
					Description: "处理完成后自动执行数据库备份白名单放行。",
					ActionType:  "http", IsActive: true,
					ConfigJSON: JSONField(`{"url":"https://httpbin.org/anything/db-backup-apply","method":"POST","headers":{"Content-Type":"application/json","X-ITSM-Action":"apply","X-ITSM-Service":"db-backup-whitelist"},"body":"{\"ticket_code\":\"{{ticket.code}}\",\"database_name\":\"{{ticket.form_data.database_name}}\",\"source_ip\":\"{{ticket.form_data.source_ip}}\",\"whitelist_window\":\"{{ticket.form_data.whitelist_window}}\"}","timeout":10,"retries":0}`),
				},
			},
		},
		{
...
处理人完成处理后流程结束。`,
		...
		{
			Name:             "多角色并签申请",
			Code:             "multi-role-parallel-approval",
			Description:      "用于验证多角色并签审批场景：网络管理员与安全管理员并行审批汇聚后由运维管理员终审。",
			CatalogCode:      "infra-network:network",
			SLACode:          "standard",
			IntakeFormSchema: multiRoleParallelApprovalFormSchema,
			WorkflowJSON:     multiRoleParallelApprovalWorkflowJSON,
			CollaborationSpec: `员工在 IT 服务台提交多角色并签申请时，服务台需要确认申请标题、目标系统、时间窗口、申请原因和期望结果。
