Feature: VPN 开通申请 — 智能引擎确定性覆盖
  ...
  通过 crafted DecisionPlan 确定性验证智能引擎执行层的全部分支，不依赖 LLM 决策。
  ...
  Scenario: AI 熔断后拒绝执行新决策
    Given "vpn-requester" 已创建 VPN 工单，访问原因为 "network_support"
    When 工单 AI 失败次数设为上限并尝试决策
    Then 操作失败
    And 时间线包含 "ai_disabled" 类型事件

  Scenario: Cancel 取消有活跃审批活动的智能工单
    Given "vpn-requester" 已创建 VPN 工单，访问原因为 "network_support"
    When 执行确定性决策 type="approve" 参与者为 "network-operator"
    And 取消智能工单，原因为 "测试取消"
    Then 工单状态为 "cancelled"
    And 所有活动和指派均已取消
    And 时间线包含 "ticket_cancelled" 类型事件

  Scenario: 低置信度决策被管理员拒绝后不执行
    Given "vpn-requester" 已创建 VPN 工单，访问原因为 "network_support"
    When 创建确定性待确认决策 type="approve"
    Then 当前活动状态为 "pending_approval"
    When 管理员拒绝当前待确认决策
    Then 当前活动状态为 "rejected"
