  ...
  "items": [
    {
      ...
      "message": "string",
      "location": {
        "kind": "collaboration_spec|workflow_node|workflow_edge|action",
        "path": "string",
        "refId": "string(可选)"
      },
...
5. 运行时配置有效性由系统确定性检查负责；不要因为输入里未出现校验代码、校验逻辑、用户表数据、存储路径或基础设施说明，就推断“缺少校验”“缺少配置”。
6. 不要输出 runtime_config 类问题；runtime_config 只允许系统确定性检查使用，不能由你自行新增。
7. 不要检查审计日志存储、日志路径、日志服务、数据库、文件系统或其他基础设施可用性；这些不属于单个服务流程的发布健康诊断范围。
8. 不要输出 fallbackAssignee / fallback_assignee / 兜底处理人校验类问题；只有输入中已经给出明确无效状态时才可引用该状态。
9. 不要输出“未验证参与者配置”“缺少参与者校验”等系统能力判断；参与者不一致必须定位到 workflow_node，并在 evidence 中同时给出该节点的实际 participant 与协作规范要求的期望 participant。
10. 如果没有风险，返回 status=pass 且 items=[]。
