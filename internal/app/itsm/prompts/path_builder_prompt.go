...
2. **rejected 出边**：data.outcome="rejected"，连接协作规范定义的驳回恢复节点；如果协作规范未提及驳回处理方式，则连接到统一的驳回结束终态

**同一个 process 节点的 approved 和 rejected 必须指向不同的目标节点**，因为通过和驳回是不同业务结果。但不同 process 节点可以复用相同业务终态：多个通过分支如果本质都是完成，应汇入同一个 end_completed；多个驳回分支如果本质都是驳回关闭，应汇入同一个 end_rejected。

示例——假设 process 节点 id 为 node_4，通过后进入 node_5，驳回后结束：

{"id": "edge_a", "source": "node_4", "target": "node_5",           "data": {"outcome": "approved"}},
{"id": "edge_b", "source": "node_4", "target": "end_rejected",      "data": {"outcome": "rejected"}}

这是结构性要求，无论协作规范是否提及驳回，rejected 出边都不可省略。

### 结束节点复用规则

end 节点表达业务终态，不表达“从哪个岗位、部门或分支过来”。不要因为多个处理人、多个岗位或多个通过分支，就生成多个等价结束节点。

默认复用两个公共终态：
- end_completed：成功完成、通过后结束、正常闭环
- end_rejected：驳回结束、拒绝关闭、未通过闭环

多个 process 节点的 approved 出边如果最终语义都是完成，应全部指向同一个 end_completed。多个 process 节点的 rejected 出边如果最终语义都是驳回结束，应全部指向同一个 end_rejected。不要生成“结束（网络管理员通过）”“结束（运维管理员通过）”“结束（信息安全管理员通过）”这类仅按来源岗位命名的重复终点。

只有协作规范明确存在不同业务终态时才拆分 end，例如取消关闭、超时关闭、失败终止、驳回后返工完成、驳回后直接关闭。即使需要拆分，也必须按终态语义命名，而不是按来源节点命名。
