...
| action | 自动动作节点（webhook/脚本） | label, nodeType, action_id (关联可用动作) |
...
| wait | 等待节点（定时/信号） | label, nodeType, wait_mode(signal/timer), duration(如 "2h") |

**重要**：每个节点的 data 中必须包含 nodeType 字段，值与外层 type 一致。

### action 节点规则

action 节点只能在“可用动作列表”明确提供了动作 id，且协作规范明确要求把该动作编排进参考路径时生成。action_id 必须是可用动作列表中的数字 id，不能使用字符串 code、动作名称或模型自行编造的标识。

如果没有可用动作列表，或协作规范只是写“人工执行开通操作”“IT 执行处理”“管理员处理”，不要生成 action 节点，应使用 process/notify/end 表达参考路径。
