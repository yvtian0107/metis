...
The seed logic SHALL be split into two functions: `seed.Install()` for first-time installation and `seed.Sync()` for subsequent startups. The `seed` CLI subcommand and `create-admin` CLI subcommand SHALL be removed.
...
**Reason**: Replaced by install wizard. Seed logic is now called programmatically during installation (`seed.Install()`) and on startup (`seed.Sync()`).
**Migration**: No manual seed command needed. Installation triggers full seed. Startup triggers incremental sync.

### Requirement: User migration in seed
**Reason**: Legacy migration from old Role string field to RoleID foreign key is no longer needed. No existing deployments to migrate.
**Migration**: None required — fresh installs only.
