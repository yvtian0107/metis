...
writeFormBindings() in variable_writer.go SHALL accept a currentNodeID parameter. For each form field being written, the function SHALL check field.Permissions[currentNodeID]. If the permission is "readonly" or "hidden", the field value SHALL be silently skipped (not written to process variables). If the permissions map is absent or has no entry for currentNodeID, the field SHALL default to "editable" (written normally). A warning log SHALL be emitted when a field write is skipped due to permission.
...
- **THEN** the field value is written normally (backward compatible)
...
The field property editor in FormDesigner SHALL include a "节点权限" (Node Permissions) section. This section SHALL display a list of workflow node IDs (loaded from the current workflow context) with a dropdown for each: "editable" | "readonly" | "hidden". Changes SHALL update the field's permissions map. The section SHALL only be visible when the designer is opened in workflow-node context (not standalone form editing).
...
- **WHEN** FormDesigner is opened standalone (e.g., from form management page)
...
- **THEN** the entry is removed from field.permissions (defaults to editable at runtime)
