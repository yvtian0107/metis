...
该工具 SHALL 搜索服务关联的知识库，返回与查询相关的知识片段。复用现有 `KnowledgeSearcher` 接口。
...
#### Scenario: 搜索有结果
- **WHEN** Agent 调用 `decision.knowledge_search` 且知识库中有匹配内容
...
#### Scenario: 服务无关联知识库
- **WHEN** Agent 调用 `decision.knowledge_search` 但当前服务未绑定任何知识库
