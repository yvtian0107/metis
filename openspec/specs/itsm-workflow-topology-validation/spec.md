# Capability: itsm-workflow-topology-validation

## Purpose

ITSM 工作流拓扑校验扩展，包括环路、死端、参与人类型与错误级别分层。

## Requirements

### Requirement: Cycle detection in workflow graph
ValidateWorkflow() SHALL detect circular paths in the workflow graph using DFS coloring. When a cycle is detected, it SHALL return a blocking-level ValidationError with the cycle path description.

#### Scenario: Workflow with no cycles passes validation
- **WHEN** ValidateWorkflow() is called on a workflow with nodes A->B->C->End
- **THEN** no cycle-related ValidationError is returned

#### Scenario: Workflow with direct cycle detected
- **WHEN** ValidateWorkflow() is called on a workflow where node B has an edge back to node A
...
ValidateWorkflow() SHALL verify that all non-end nodes can reach at least one end node. Detection SHALL use reverse BFS from the end node. Nodes unreachable from the end node SHALL produce a blocking ValidationError.
